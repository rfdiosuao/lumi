...
name: luminode_agent
description: >
  控制 Luminode 桌面智能体的启动、暂停和状态查询。
  当用户需要启动 / 暂停 Luminode 消息自动回复智能体，或查询其运行状态时使用此 skill。
  智能体应用需要提前启动并保持运行。
metadata: {"openclaw":{"emoji":"👁️","requires":{"os":["macos","windows"]}}}
---

# Luminode Agent 控制 Skill

本 skill 通过本地 HTTP API 控制 Luminode 桌面智能体。API 仅监听 `127.0.0.1:12680`。

## 前置条件

- Luminode 桌面客户端必须已经启动并运行
