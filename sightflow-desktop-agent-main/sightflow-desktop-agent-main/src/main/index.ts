...
import { HttpApiServer } from '../core/http-api-server'
...
type ArkRuntimeConfig = {
  apiKey: string
  model: string
  baseURL: string
}

function cleanRuntimeString(value: unknown): string {
  return typeof value === 'string' ? value.trim() : ''
}

function normalizeArkBaseURL(value: unknown): string {
  return (cleanRuntimeString(value) || FIXED_ARK_BASE_URL).replace(/\/+$/, '')
}

function resolveArkRuntimeConfig(settings: AppSettings, overrides: Record<string, any> = {}): ArkRuntimeConfig {
  ...
  return {
    apiKey:
      cleanRuntimeString(overrides.apiKey) ||
      cleanRuntimeString(settings.vision?.apiKey) ||
      cleanRuntimeString(providerConfig.apiKey),
...
let standaloneHttpApiDevice: RPADevice | null = null

// ── Launcher sidecar：Lumi/OpenClaw 启动器遥控接入点 ──
// 启动器以 `--luminode-sidecar --port --token --app-type --api-key` 拉起本 agent 时，
// 立即开放 token 保护的本地 HTTP API（/screenshot /click /type /wechat/send /wechat/unread …），
// 复用本项目（-main）的 RPADevice 引擎，使启动器的桌面控制照常工作，同时享受 -main 更全的能力。
...
// 启动器历史上传 appType='weixin'，而 -main 用 'wechat'；这里归一化，并放行 -main 支持的其它渠道。
function normalizeLauncherAppType(raw: string): AppType {
  const v = (raw || '').toLowerCase()
  if (v === 'weixin' || v === 'wechat' || v === '') return 'wechat'
  const allowed: string[] = ['wechat', 'wework', 'dingtalk', 'lark', 'slack', 'telegram', 'generic']
  return allowed.includes(v) ? (v as AppType) : 'wechat'
}
...
function maybeStartLauncherSidecar(): void {
  ...
  httpApiServer.start().catch((err: unknown) => {
...
app.whenReady().then(async () => {
  ...
  // ── Launcher sidecar HTTP API（启动器桌面控制接入点：截图/点击/微信发送/未读）──
...
function normalizeSettings(raw: any): AppSettings {
  ...
  const oldBaseURL =
    typeof raw?.baseURL === 'string' && raw.baseURL ? raw.baseURL : FIXED_ARK_BASE_URL
