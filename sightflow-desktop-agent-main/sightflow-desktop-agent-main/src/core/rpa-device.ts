...
export class RPADevice implements DesktopDevice {
  ...
  setApiKey(apiKey: string, config?: { baseURL?: string; model?: string }): void {
    if (!apiKey) return
    // 允许同时指定 baseURL/model（启动器 sidecar 模式会把网关地址+模型一起传进来），
    // 否则视觉客户端会回退到默认火山地址，拿网关 token 直连会 401「key 格式不对」。
