    ...
    configSchema: {
      fields: [
        ...
        },
        {
          ...
          type: 'text',
          required: true,
          defaultValue: DEFAULT_ARK_MODEL
        },
        {
          key: 'baseURL',
          label: 'Base URL',
          ...
          defaultValue: DEFAULT_ARK_BASE_URL
        },
        {
          key: 'systemPrompt',
          label: '系统提示词',
...
function SettingsPanel() {
  ...
  const [providerModel, setProviderModel] = useState(DEFAULT_ARK_MODEL)
  const [providerBaseURL, setProviderBaseURL] = useState(DEFAULT_ARK_BASE_URL)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (settings) {
        ...
        const config = settings.chatProvider?.config || {}
        setProviderModel(
          typeof config.model === 'string' && config.model ? config.model : DEFAULT_ARK_MODEL
        )
        setProviderBaseURL(
  ...
  const handleSaveVision = useCallback(async () => {
    const payload = {
      ...
      chatProvider: {
        config: {
          model: providerModel.trim() || DEFAULT_ARK_MODEL,
          baseURL: providerBaseURL.trim().replace(/\/+$/, '') || DEFAULT_ARK_BASE_URL
        }
      }
    }
  ...
  }, [providerBaseURL, providerModel, visionApiKey])
