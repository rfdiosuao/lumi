...
Luminode 桌面端把“截图分析并生成回复”的聊天能力抽象为 Provider。外部技术使用者只需要提供一份 `manifest.json` 和一个 bundle 入口文件，应用会负责下载安装、读取配置、传入聊天截图，并消费 Provider 返回的事件。
...
```text
file:///path/to/luminode-desktop/resources/providers/volcengine-ark/manifest.json
```
