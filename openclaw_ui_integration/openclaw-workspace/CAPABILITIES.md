...
OpenClaw ships with a lightweight office skill pack under `skills/`:

- `openclaw-meeting-notes`: meeting minutes, decisions, action items, and follow-up messages.
- `openclaw-weekly-report`: daily reports, weekly reports, progress updates, and boss-facing summaries.
- `openclaw-pdf-ocr`: PDF, screenshot, scanned document, invoice, form, and OCR-oriented extraction.
- `openclaw-excel-analysis`: spreadsheet cleanup, KPI analysis, anomalies, formulas, and management summaries.
- `openclaw-presentation-builder`: PPT outlines, HTML slide decks, business presentations, and competitor decks.
- `openclaw-file-organizer`: safe office file classification, folder plans, duplicate checks, and cleanup proposals.

Use these as operating recipes for office workflows. They do not authorize destructive file changes by themselves; follow the file safety rules in each skill.
