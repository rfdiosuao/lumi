...
import { Camera, MessageCircleMore, RefreshCcw, Send, ShieldCheck, SquareTerminal, SquareStack, StopCircle } from 'lucide-react';
    ...
    <div className="page-grid">
      <section className="hero-band">
        <div className="hero-copy">
          ...
          <h1>lumi 桌面控制台</h1>
          <p>统一托管内置 Luminode 代理，支持 Windows 与 macOS 桌面截图、微信未读检测和受控自动回复。</p>
        </div>
      ...
      <section className="stats-grid">
        ...
        <StatCard label="内置代理" value={data?.present ? '已发现' : '未找到'} hint={shortPath(resolvedAgentDir || 'agents/luminode-desktop')} tone={data?.present ? 'ok' : 'warn'} />
        ...
        <section className="content-grid content-grid-desktop">
          <Panel className="surface-panel">
            <SectionHeader
              eyebrow="状态"
              title="桌面代理健康状态"
              subtitle="启动器通过 /api/desktop-agent/status 管理内置 Luminode sidecar。"
              action={<Chip tone={data.source === 'live' ? 'ok' : 'warn'}>{sourceLabel(data.source)}</Chip>}
            />
            <div className="detail-stack">
              <div className="detail-row"><span className="detail-label">代理目录</span><span className="detail-value">{resolvedAgentDir || '未找到内置 agents/luminode-desktop'}</span></div>
          ...
          <Panel className="surface-panel rpa-actions-panel">
            ...
            <div className="button-row">
              <Button variant="secondary" icon={Camera} onClick={handleScreenshot} disabled={!data.apiReady}>截图</Button>
              <Button variant="secondary" icon={MessageCircleMore} onClick={() => handleAction('/api/desktop-agent/wechat/unread', {})} disabled={!data.apiReady}>未读</Button>
              <Button variant="success" icon={Send} onClick={() => handleAction('/api/desktop-agent/wechat/send', { text: message, confirmed: true })} disabled={!canSend}>发送</Button>
            </div>
            {!canSend ? (
              <InlineState
                tone="warn"
          ...
          <Panel className="surface-panel surface-panel-wide">
            <SectionHeader eyebrow="配置" title="代理配置" subtitle="留空目录时自动查找内置 agents/luminode-desktop，Mac 包会解析 Luminode.app。" />
            <details className="settings-details">
              <summary>编辑桌面代理配置</summary>
              <div className="desktop-config-stack">
                <div className="form-grid">
                  <Field label="Agent 目录" hint="可留空使用内置代理"><Input value={configDraft.agentDir || ''} placeholder="agents/luminode-desktop 或 Luminode.app" onChange={(event) => setConfigDraft((state) => ({ ...state, agentDir: event.target.value }))} /></Field>
