...
function detectOwner() {
  if (process.env.OPENCLAW_RELEASE_REPOSITORY) return process.env.OPENCLAW_RELEASE_REPOSITORY;
  return "rfdiosuao/openclaw-runtime-layers";
}
...
async function main() {
  ...
  const tag = args.tag || process.env.OPENCLAW_RELEASE_TAG || `v${version}`;
