...
const loginArgs = ['models', 'auth', 'login', '--provider', 'openai', '--set-default'];
if (action === 'openai-device-code') {
  loginArgs.push('--device-code');
} else if (action !== 'openai-browser') {
  fail(`Unknown auth action: ${action}`);
} else {
  loginArgs.push('--method', 'oauth');
}
...
function tmpFile(extension) {
  ...
  return path.join(dir, `openai-codex-login-${action}${extension}`);
}
...
function writeWindowsScript() {
  ...
  const lines = [
    ...
    "Write-Host 'OpenClaw OpenAI Codex login'",
    "Write-Host 'A browser login page should open automatically.'",
    "Write-Host 'After signing in, keep this window open until OpenClaw saves the auth profile.'",
    `& ${psQuote(nodeExe)} ${[openclawMjs, ...loginArgs].map(psQuote).join(' ')}`,
...
function writeUnixScript() {
  ...
  const lines = [
    ...
    `${shQuote(nodeExe)} ${[openclawMjs, ...loginArgs].map(shQuote).join(' ')}`,
    'code=$?',
